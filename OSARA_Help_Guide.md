---
title: "OSARA Help Guide"
subtitle: "The official OSARA documentation, arranged for reading"
date: 2026-08-02
lang: en
---
...
This guide gathers OSARA's own documentation, as it stood on 2 August 2026.

OSARA -- Open Source Accessibility for the REAPER Application -- is the extension that makes
REAPER usable with a screen reader. It is published by James Teh and other contributors
under NV Access, and it is not a Cockos product; REAPER's own documentation is a separate
volume in this collection.

**A word about what this volume is.** OSARA does not publish a help centre. It publishes a
readme -- and that readme is the documentation. The words here are exactly those, taken
from the project's own repository in their original Markdown, so nothing passed through a
conversion that could alter it. What this guide adds is arrangement: a table of contents,
consistent heading levels, and each topic reachable as its own heading.

**What OSARA itself says is missing.** The readme states plainly that it documents what is
specific to OSARA and does not attempt to explain how a screen reader user can best
understand and use REAPER, and it points readers to the REAPER Accessibility Wiki for that.
That wiki is written by the community rather than by OSARA's authors. It is not in this
edition. It is worth knowing that the project itself considers it necessary reading.

Set aside, because they are written for people working on OSARA rather than people using
it: Building, which covers compiling from source; Translating, which covers contributing
translations; and Contributors, which is a list of names.

This document holds 3 categories, 25 articles and about 10,115 words.
...
- [Download and Installation](#download-and-installation)
    ...
    - [Windows](#windows)
- [Usage](#usage)
    - [Accessing Controls for Sends/Receives/Outputs in the Track I/O Window](#accessing-controls-for-sends-receives-outputs-in-the-track-i-o-window)
    - [Accessing FX Plug-in Windows](#accessing-fx-plug-in-windows)
    - [Actions that report, but are not directly mapped to keyboard shortcuts](#actions-that-report-but-are-not-directly-mapped-to-keyboard-shortcuts)
    - [Automation Items](#automation-items)
    - [Configuration](#configuration)
    - [Context Menus](#context-menus)
    - [Manually Moving Stretch Markers](#manually-moving-stretch-markers)
    - [Miscellaneous Actions](#miscellaneous-actions)
    - [Muting OSARA Messages in Custom/Cycle Actions](#muting-osara-messages-in-custom-cycle-actions)
    - [Noncontiguous Selection](#noncontiguous-selection)
    - [Note Preview in the MIDI Event List (Windows Only)](#note-preview-in-the-midi-event-list-windows-only)
    - [Notes and Chords in the MIDI Editor](#notes-and-chords-in-the-midi-editor)
    - [Parameter Lists](#parameter-lists)
    - [Peak Watcher](#peak-watcher)
    - [Reading Current Peaks](#reading-current-peaks)
    - [Searching and Navigating FX Presets Without Activating Them (Windows Only)](#searching-and-navigating-fx-presets-without-activating-them-windows-only)
    - [Shortcut Help](#shortcut-help)
    - [Supported REAPER and Extension Actions](#supported-reaper-and-extension-actions)
