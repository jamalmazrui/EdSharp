...
The last page of the installer offers components as checkboxes, ticked or not according to what EdSharp itself gains from them. Python and the document tools, which give rich PDF conversion and the thesaurus, are ticked: together they cost about 155 megabytes and every user benefits. Git with the GitHub command line, Node.js, and Ollama with its chat model are not ticked; each serves a real feature, but each serves some people and not others, and Ollama alone costs about three gigabytes. The boxes are ordered by what they will do: those that install something come first, then those that update something, and last anything already current, offered only for repair. Each label says whether that tool is missing, out of date, or already current, with version numbers. They run after you press Finish, each in its own window that shows progress and closes when it is done; none of them ever waits for a keypress. When they have all finished, a single Results box reports everything: the JAWS scripts, the NVDA add-on, pandoc, and each optional install by name, saying what is installed and which version, or what to run later for anything that is not. The summary is saved as EdSharp_setup_summary.txt in the logs folder under your local application data, and you can see it again at any time by running summarizeSetup.cmd from the program folder.
...
Press F7 to check spelling. EdSharp uses Hunspell, the free spelling engine behind LibreOffice and Firefox, with a dictionary that ships in the Dictionaries folder beside the program. Nothing is registered with Windows and no Microsoft Word is needed. Words you add go in Dictionary.txt in your EdSharp settings folder, a plain list you can edit by hand. The SpellChecker setting chooses a different engine: "Windows" for the checker built into Windows, which knows the words you have taught Windows in other programs, or "Word" for the Microsoft Word dialog. If text is selected, only that text is checked; otherwise the whole document is.
...
When the window holds a source file, F12 sends your question to a model trained on code rather than the general chat model, in the same way Compile picks a compiler by the kind of file you are editing. The installer offers qwen2.5-coder:7b as a checkbox, about five gigabytes, beside the Ollama box; tick it and the change happens by itself.

What it is good at is anything about the file in front of you: explain this error message, what does this method do, write a function that does such and such, review this for mistakes, convert this loop to the other style, add comments. That is work where the whole subject fits in the question, which is where a model this size does well. What it cannot do is reason about a program spread over many files, because it sees only what you send it, and a large file may not fit at once; a method or a class is the practical unit.

Say which framework you are targeting when it matters. Most C# written today targets .NET rather than the .NET Framework that EdSharp itself uses, so a question that does not say so may come back with code that will not compile here. The CodeModel option names a different model if you prefer one.

### Translating

Press Alt+Shift+F7 to translate the selection, or the whole document when nothing is selected. Choose the language you are translating from and the one you are translating to, and the translation opens in a new window, leaving the original untouched. Your last pair of languages is remembered.

The work is done by the same AI model on your computer that Chat with AI uses, so nothing is sent over the internet, there is no account, and there are no limits on how much you translate. Quality is good between the widely written languages, Spanish, French, German, Portuguese and Italian among them, and useful but less reliable for languages with less material behind them.

A larger model translates better, at the cost of disk space and time. The installer offers qwen2.5:7b as a checkbox, about five gigabytes, beside the Ollama box; tick it and translation uses it automatically, while Chat with AI keeps llama3.2, the smaller and quicker model. Nothing needs setting. If you prefer a different model, name it in the TranslateModel option with Configuration Options, Alt+Shift+C.

### Chat with AI

Press F12 to talk with an AI model running on your own computer. Whatever is selected, or the whole document when nothing is selected, is sent along with the instruction you type, and you type the prompt in a box that keeps its lines -- Enter starts a new line, Control+Enter submits -- and the answer opens in a plain new window, titled by EdSharp like any unnamed document. The original window is never changed, and the answer arrives as an ordinary document: read it with your usual keys, save it, or convert it. This one command covers both conversation, by asking a question in an empty document, and transformation, by selecting text and saying what to do with it, such as summarize this, translate this to Spanish, or add docstrings to these functions. Everything runs locally through Ollama: your text never leaves the machine, there is no account and no charge, and one Ollama installation is shared by every application that uses it. The last page of EdSharp's installer offers Ollama and a starter chat model as a checkbox; the OllamaModel setting picks a different model whenever you like.

### Compilers

Press Control+Shift+F5 to pick the compiler for your work. The shipped set covers what Windows developers realistically use today: C#, Python, JavaScript through Node.js, PowerShell, JAWS Script, VBScript, JScript .NET, and HTML Tidy for checking web pages, plus Default for plain editing. Each compiler is defined by a section named Compiler followed by its name in EdSharp.inix, with one clearly named key per setting: CompileCommand, JumpPosition, AbbreviateOutput, NavigatePart, QuotePrefix, ExtensionDefault, IndentUnit, and GoToEnvironment. Picking a compiler brings all of them, so choosing Python also brings four space indentation and Python's own prompt, while choosing JavaScript brings two spaces and Node's shell; a document that already has indentation of its own still governs itself. Control+Shift+G opens that shell: Python's interpreter with your file already run, Node's read-evaluate-print shell, or PowerShell left open after running the file. Two of them are built into EdSharp, for the languages Windows gives no interpreter: a JScript console for writing snippets and a C# console, both opening with the editor window in scope as frm and its text box as rtb, so a line tried at the prompt does to your document exactly what the same line does in a snippet. The C# one compiles each line with the compiler named in the compiler settings and loads it into EdSharp, which is why it can reach the live document; a line ending in a semicolon is kept for the rest of the session, and anything else is printed. A language with no such shell, JAWS script for one, says so rather than starting something unrelated. Values are kept exactly as written and may span multiple lines, so regular expressions are easy to read and edit. Every expression in the shipped set was checked against the tool's real message format, so Compile, Control+F5, jumps to the true error line and column. Two things make that practical to work with. Noise is removed before you hear anything, so the announcement starts with an error rather than with your own file path, which Python and most compilers repeat in every message. And when a tool reports several problems, EdSharp goes to the one earliest in the file, whatever order the tool printed them in, and begins speaking there: fix it, press Control+F5 again, and the next problem is waiting, so you work down the file from the top. The full output, including anything skipped, is always available with Review Output. Older compilers that no longer earn a place, such as Perl, Ruby, and PowerBASIC, were retired from the shipped list; any of them can return as a private section, and old-style packed entries still work for compilers that have no section.

EdSharp aims to be the most friendly and productive editor for screen reader users writing Python. The pieces working together: the PyBrace command, Alt+Shift+LeftBracket, converts indented Python to a flat form in which structure is spoken text rather than counted spaces, and PyDent, Alt+LeftBracket, converts back, rebuilding clean indentation and leaving a # end comment at the close of every block so the ends of functions, classes, and loops are audible while you read. Both directions now track strings and brackets character by character, so dictionary literals, docstrings, f-strings, and line continuations survive the round trip exactly. The Format Code command routes a .py file through this same engine to normalize its indentation. Compile, Control+F5, runs a Python file with the python on your path even before any compiler is picked, jumping to the traceback's line number; the installer offers the latest official Python for Windows as an optional checkbox. That is the build from python.org, not the Microsoft Store edition, which installs into a sandboxed folder and behaves differently in ways that break tools. Windows also puts a stub named python on your path that only advertises the Store; EdSharp ignores it and looks for the real Python by its own location, and installPython.cmd does the same. Indent Mode, block navigation, and Infer Indent complete the set.
...
EdSharp speaks only what your screen reader cannot already tell you. Window titles, and the name, role and value of whatever has keyboard focus, are announced by JAWS, NVDA or Narrator on their own, so EdSharp stays quiet about them; what it does speak is the answer to a command, such as a count, a level, or the text a navigation command reached. Each message goes to one voice only: JAWS if it is running, otherwise NVDA, otherwise a system notification that Narrator reads.

EdSharp writes a log for each session, named EdSharp followed by a time stamp, in the logs folder under your local application data, beside the setup log. It records the version, the environment, and every outside command EdSharp runs with its exit code, so a failed conversion or compile can be diagnosed from the log rather than guessed at. The newest thirty session logs are kept. Everything EdSharp records goes in this one folder: the setup log, the session logs, the setup summary, and the small files the installer and its helper scripts use to talk to each other. Each line is written the moment it happens rather than held in memory, so a log is complete even when the thing being logged is what went wrong. Press Control+F12, Copy Log, to put the current log's path on the clipboard as both a file and text, ready to paste as a mail attachment.
...
The Transform Files command, Alt+Equals, applies a saved job of search and replace tasks to one or more files -- typically to massage data or formatting in predictable ways.  The current document holds the list of files to process, one path per line, and EdSharp prompts for the job file.  A job is an .inix file (it may carry a .ini or .inix extension) with one bracketed section per task.  Each section has a Find key (the regular expression to match) and then either replaces or extracts.  A Replace key gives the replacement text, in which \n, \t, and similar escapes are honored, the usual .NET group references such as $1 work, and $# inserts the running match count.  To delete the matched text, simply leave the Replace key out of the section:  a task with a Find but no Replace removes every match.  If you prefer to state the empty replacement explicitly, write Replace="" -- a pair of double quotes with nothing between them.  Do not write a bare Replace= with nothing after the equals sign, since that begins a multi-line value rather than an empty one.  Alternatively, an Extract key set to true collects each match to the clipboard without changing the file, separated by the optional Divider key (a form feed and newline by default).  An optional Options key takes a comma-separated list of .NET regular expression option names such as multiline, ignorecase, or singleline.  Any single-line value may be surrounded by double quotes, as in traditional .ini files.  The quotes are delimiters rather than content, and everything between them is kept exactly, so quoting is how you preserve a space that would otherwise be trimmed away:  Replace=" dog" keeps the leading space, and Replace="dog " keeps the trailing one.  Quoting is optional; an unquoted value works as before.  Because the .inix format supports multi-line values, a Find or Replace expression may span several lines.  This is the same job format processed by the companion Regexer tool.  When you run a job, EdSharp offers Test (count matches only), Run (apply the changes), and Verbose (apply with per-task detail) modes.
...
Chat with AI=F12, Send the selection, or the whole document when nothing is selected, together with your typed instruction to a local AI model through Ollama; the document travels with the question only when the instruction refers to it, such as summarize or translate this, so a general question is answered quickly and on its own; the answer opens in a plain new unnamed window, leaving the original untouched; works as plain conversation in an empty document or as text transformation with a selection; the model, address, and timeout are the OllamaModel, OllamaUrl, and OllamaTimeout settings
Chat about Document=Shift+F12, Ask the AI about the open text whatever the wording of the instruction: the selection when text is selected, the whole document when it is not; use this when F12 would treat your instruction as a general question
