...
Chat with AI=F12, Send the selection, or the whole document when nothing is selected, together with your typed instruction to a local AI model through Ollama; the answer opens in a plain new unnamed window, leaving the original untouched; works as plain conversation in an empty document or as text transformation with a selection; the model, address, and timeout are the OllamaModel, OllamaUrl, and OllamaTimeout settings
