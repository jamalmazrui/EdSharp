//EdSharp 3.1
//March 22, 2009
//Copyright 2007 - 2009 by Jamal Mazrui
...
[assembly: AssemblyCopyright("Copyright 2007 - 2008 by Jamal Mazrui")]
...
Util.String2File(Util.Convert2WinLineBreak(this.RTB.Text), sFile, ref en);
}
this.RTB.Modified = false;
...
public ToolStripMenuItem menuQuery, menuQueryAddress, menuQueryBraces, menuQueryBlock, menuQueryIndent, menuQueryPath, menuQueryTopic, menuQueryYield, menuQueryStatus, menuQueryCompiler, menuQuerySelected, menuQueryChunk, menuQueryReadAll, menuQueryWindowsOpen, menuQueryClipboard, menuQueryTime, menuQueryStyles, menuQuerySelectionFont;
public ToolStripMenuItem menuMisc, menuMiscSetDefaultFont, menuMiscConfigurationOptions, menuMiscManualOptions, menuMiscResetConfiguration, menuMiscGoToFolder, menuMiscGoToSpecialFolder, menuMiscWordWrap, menuMiscUnwrap, menuMiscExtraSpeechToggle, menuMiscExtraSpeechLog, menuMiscEnvironmentVariables, menuMiscSpellCheck, menuMiscThesaurus, menuMiscWebCheck, menuMiscGuardDocument, menuMiscNoGuard, menuMiscPyBrace, menuMiscPyDent, menuMiscInferIndent, menuMiscRepeatLine, menuMiscSectionBreak, menuMiscPathToClipboard, menuMiscPathList, menuMiscInsertTime, menuMiscCalculateDate, menuMiscHTMLFormat, menuMiscTextConvert, menuMiscTextCombine, menuMiscTextContents, menuMiscYieldWithRegExp, menuMiscExtractWithRegExp, menuMiscRunAtCursor, menuMiscSpecialCharacter, menuMiscEvaluateExpression, menuMiscReplaceTokens, menuMiscTransformFiles, menuMiscGoToEnvironment, menuMiscCompile, menuMiscPickCompiler, menuMiscPromptCommand, menuMiscReviewOutput, menuMiscSaveSnippet, menuMiscInvokeSnippet, menuMiscViewSnippet, menuMiscKeepUniqueLines, menuMiscNumberLines, menuMiscOrderLines, menuMiscReverseLines, menuMiscListDifferentLines, menuMiscQueryCommonLines, menuMiscExplorerFolder, menuMiscCommandPrompt, menuMiscWebDownload;
...
menuFileOpenOtherFormat = CreateMenuItem("OpenOtherFormat ...", "Control+Shift+O", menuItem_Click, "frame speak");
...
menuFileMailBody = CreateMenuItem("&Mail ...", "Control+M", menuItem_Click, "child speak");
menuFileMailAttach = CreateMenuItem("Mail as Attachment ...", "Control+Shift+M", menuItem_Click, "child speak");
...
menuDeleteTrimBlanks = CreateMenuItem("TrimBlanks", "Control+Shift+Enter", menuItem_Click, "child speak");
...
menuNavigateNextJustify = CreateMenuItem("Next Justify", "Control+OemCloseBrackets", menuItem_Click, "child silent");
menuNavigatePriorJustify = CreateMenuItem("Prior Justify", "Control+OemOpenBrackets", menuItem_Click, "child silent");
...
menuQueryCompiler = CreateMenuItem("Compiler", "Alt+D0", menuItem_Click, "child silent");
...
menuQuerySelectionFont = CreateMenuItem("Selection Font", "Alt+OemMinus", menuItem_Click, "child silent");
menuQuery.DropDownItems.AddRange(new ToolStripItem[] {menuQueryAddress, menuQueryBraces, menuQueryBlock, menuQueryIndent, menuQueryPath, menuQueryTopic, menuQueryYield, menuQueryStatus, menuQueryCompiler, menuQuerySelected, menuQueryChunk, menuQueryReadAll, menuQueryWindowsOpen, menuQueryClipboard, menuQueryTime, menuQueryStyles, menuQuerySelectionFont});
//Dialog.Show("Query.", menuQuery.DropDownItems.Count);

menuMisc = CreateMenu("&Misc");
menuMiscSetDefaultFont = CreateMenuItem("Set Default Font ...", "Alt+Shift+Oemplus", menuItem_Click, "child speak");
...
menuMiscWebCheck = CreateMenuItem("Web Check", "Alt+F7", menuItem_Click, "frame silent");
...
menuMiscReplaceTokens = CreateMenuItem("Replace Tokens or Snippet", "Control+Shift+Oemplus", menuItem_Click, "child silent");
...
menuMiscKeepUniqueLines = CreateMenuItem("Keep Unique Lines", "Alt+Shift+K", menuItem_Click, "child speak");
menuMiscNumberLines = CreateMenuItem("Number Lines ...", "Alt+Shift+N", menuItem_Click, "child silent");
menuMiscOrderLines = CreateMenuItem("Order Lines", "Alt+Shift+O", menuItem_Click, "child speak");
menuMiscReverseLines = CreateMenuItem("Reverse Lines", "Alt+Shift+Z", menuItem_Click, "child speak");
menuMiscListDifferentLines = CreateMenuItem("List Different Lines", "Alt+Shift+L", menuItem_Click, "child speak");
menuMiscQueryCommonLines = CreateMenuItem("Query Common Lines", "Alt+Shift+Q", menuItem_Click, "child speak");
menuMiscExplorerFolder = CreateMenuItem("Explorer Folder", "Alt+Oem5", menuItem_Click, "child speak");
menuMiscCommandPrompt = CreateMenuItem("Command Prompt", "Control+Oem5", menuItem_Click, "child speak");
...
menuHelpHotKeySummary = CreateMenuItem("HotKeySummary", "Alt+Shift+H", menuItem_Click, "frame speak");
...
if (s == "Y" || s == "YES") this.Shown += delegate(object o, EventArgs e) {this.WindowState = FormWindowState.Maximized; this.Activate(); Win32.SetForegroundWindow(this.Handle);};
this.Shown += delegate(object o, EventArgs e) {Util.ActivateTitle(this.Text);};
...
sDate = (sDateFormat.Length > 0) ? dt.ToString(sDateFormat) : dt.ToLongDateString();
sTime = (sTimeFormat.Length > 0) ? dt.ToString(sTimeFormat) : dt.ToShortTimeString();
...
string sSourceBody = Util.File2String(sSourceFile);
...
Util.String2FileA(sSourceBody, sSourceFile);
}
}
iLine++;
}
...
string sKeyDescription = Ini.ReadValue(App.HotkeyIniFile, "Hotkeys", sLabel, "");
if (sKeyDescription.Length == 0) sKeyDescription = Ini.ReadValue(App.HotkeyIniFile, "Hotkeys", "Say " + sLabel, "");
if (sKeyDescription.Length == 0) sKeyDescription = "No description available";
SetMessage(sLabel);
int iComma = sKeyDescription.IndexOf(",");
if (iComma == -1) AddMessage(sKeyDescription);
else {
AddMessage(sKeyDescription.Substring(0, iComma));
AddMessage(sKeyDescription.Substring(iComma + 1));
}
return;
}
...
string sRecipient = "";
Util.MailMessage(sRecipient, sSubject, sBody);
try {
...
sResult = Dialog.Input("Web Check", "Term", sText).Trim();
if (sResult.Length == 0) return;

App.WriteData("Term", sResult);
//AddMessage("Please wait");
AddMessage("Connecting");
sText = VB.WebCheck(sResult);
if (!IsEmptyWindow()) new MdiChild(this);
child = this.Child;
...
if (menuItem == menuQueryTime) AddMessage(sText);
else {
...
if (menuItem == menuQuerySelectionFont) {
...
aResults = Util.GetFilesWithExtensions(aResults, sResult);
if (aResults.Length == 0) {
AddMessage("No files!");
return;
}
...
aResults = Util.GetFilesWithExtensions(Directory.GetFiles(sDir), sResult);
iLength = aResults.Length;
...
string sOutput = Util.GetProgramOutput("cmd.exe", "/c " + sCommand);
...
if (menuItem == menuMiscKeepUniqueLines) {
string sLimitComparison = Util.Literalize(App.ReadOption("LimitComparison", "\n"));
if (rtb.SelectionLength == 0) {
AddMessage("All");
iStart = 0;
iEnd = rtb.TextLength;
...
sText = rtb.GetRange(iStart, iEnd);
aResults = Regex.Split(sText, sLimitComparison);
...
sText = String.Join(sLimitComparison, aResults);
rtb.ReplaceRange(iStart, iEnd, sText);
rtb.Index = iStart;
}

if (menuItem == menuMiscNumberLines) {
string sLimitComparison = Util.Literalize(App.ReadOption("LimitComparison", "\n"));
if (rtb.SelectionLength == 0) {
sTitle = "Number Lines All";
iStart = 0;
iEnd = rtb.TextLength;
}
else {
sTitle = "Number Lines Selected";
iStart = rtb.SelectionStart;
iEnd = iStart + rtb.SelectionLength;
}
...
if (menuItem == menuMiscOrderLines) {
string sLimitComparison = Util.Literalize(App.ReadOption("LimitComparison", "\n"));
if (rtb.SelectionLength == 0) {
AddMessage("All");
iStart = 0;
...
if (menuItem == menuMiscReverseLines) {
string sLimitComparison = Util.Literalize(App.ReadOption("LimitComparison", "\n"));
if (rtb.SelectionLength == 0) {
AddMessage("All");
iStart = 0;
...
if (menuItem == menuMiscListDifferentLines) {
string sLimitComparison = Util.Literalize(App.ReadOption("LimitComparison", "\n"));
...
string[] a = Regex.Split(sText, sLimitComparison);
...
if (menuItem == menuMiscQueryCommonLines) {
// aResults = rtb.GetRange(0, rtb.RowStart).Split('\n');
// string[] a = rtb.GetRange(rtb.RowStart, rtb.TextLength).Split('\n');
string sLimitComparison = Util.Literalize(App.ReadOption("LimitComparison", "\n"));
sText = rtb.GetRange(0, rtb.RowStart);
...
List<string[]> listLinks = VB.GetLinks(sUrl);
List<string> listFiles = new List<string>();
string sRef;
...
aResults = Util.GetFilesWithExtensions(aFiles, sResult);
...
sText = "EdSharp 3.1\n\nMarch 22, 2009\n\n";
sText += "Copyright 2007 - 2008 by Jamal Mazrui\n\nModified GPL License";
...
if (sVerb == "Open With...") Win32.OpenWith(sFile);
...
btnCancel.Click += delegate(object o, EventArgs e) {/*Util.Say("Cancel");*/ frm.Close();};
...
frm.Shown += delegate(object sender, EventArgs e) {Win32.SetForegroundWindow(frm.Handle); };
frm.ShowDialog();
frm.Dispose();
...
Button btnCancel = new Button();
btnCancel.Click += delegate(object o, EventArgs e) { /*Util.Say("Cancel");*/ sResult = ""; frm.Close();};
btnCancel.Text = "Cancel";
btnCancel.AccessibleName = btnCancel.Text;
...
object oJFW = null;
return JFWSay(sText, oJFW);
} // JFWSay method

public static bool JFWSay(string sText, object oJFW) {
try {
// if (oJFW == null) oJFW = CreateObject("JFWApi");
...
if (en == null) en = Util.GetFileEncoding(sFile);
